...
class CouponApplyDomainTest {
    ...
    void belongsTo_neitherMatch() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of("p1"), List.of(), List.of("b1"), null)
        );
        assertThat(domain.belongsTo("p2", "b2")).isFalse();
    }

    @ParameterizedTest
    @NullAndEmptySource
    @DisplayName("대상이 blank 이면 소속이 아니다")
    void belongsTo_blankTarget(final String blank) {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of("p1"), List.of(), List.of("b1"), null)
        );
        assertThat(domain.belongsTo(blank, blank)).isFalse();
    }

    @Test
    @DisplayName("적용 제한이 없으면 통과한다")
    void isSatisfied_noRestrictions() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of(), List.of(), List.of(), null)
        );
        assertThat(domain.isSatisfied("p1", "c1", "b1", "s1")).isTrue();
    }

    @Test
    @DisplayName("허용 상품이면 통과한다")
    void isSatisfied_productAllowed() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of("p1"), List.of(), List.of(), null)
        );
        assertThat(domain.isSatisfied("p1", "c1", "b1", null)).isTrue();
    }

    @Test
    @DisplayName("허용되지 않은 상품이면 실패한다")
    void isSatisfied_productDenied() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of("p1"), List.of(), List.of(), null)
        );
        assertThat(domain.isSatisfied("p2", "c1", "b1", null)).isFalse();
    }

    @Test
    @DisplayName("허용되지 않은 카테고리면 실패한다")
    void isSatisfied_categoryDenied() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of(), List.of("c1"), List.of(), null)
        );
        assertThat(domain.isSatisfied("p1", "c2", "b1", null)).isFalse();
    }

    @Test
    @DisplayName("허용되지 않은 브랜드면 실패한다")
    void isSatisfied_brandDenied() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of(), List.of(), List.of("b1"), null)
        );
        assertThat(domain.isSatisfied("p1", "c1", "b2", null)).isFalse();
    }

    @Test
    @DisplayName("세그먼트가 다르면 실패한다")
    void isSatisfied_segmentMismatch() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of(), List.of(), List.of(), "vip")
        );
        assertThat(domain.isSatisfied("p1", "c1", "b1", "normal")).isFalse();
    }

    @Test
    @DisplayName("세그먼트가 일치하면 통과한다")
    void isSatisfied_segmentMatch() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of(), List.of(), List.of(), "vip")
        );
        assertThat(domain.isSatisfied("p1", "c1", "b1", "vip")).isTrue();
    }

    @Test
    @DisplayName("정책에 세그먼트가 있으면 요청 세그먼트가 blank 여도 실패한다")
    void isSatisfied_blankRequestSegmentFails() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of(), List.of(), List.of(), "vip")
        );
        assertThat(domain.isSatisfied("p1", "c1", "b1", null)).isFalse();
        assertThat(domain.isSatisfied("p1", "c1", "b1", "  ")).isFalse();
    }

    @Test
    @DisplayName("상품·카테고리·브랜드·세그먼트를 모두 만족해야 통과한다")
    void isSatisfied_allMustPass() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of("p1"), List.of("c1"), List.of("b1"), "vip")
        );
        assertThat(domain.isSatisfied("p1", "c1", "b1", "vip")).isTrue();
        assertThat(domain.isSatisfied("p1", "c1", "b2", "vip")).isFalse();
    }

    @Test
    @DisplayName("제한이 있어도 대상이 blank 이면 실패한다")
    void isSatisfied_restrictedButBlankTarget() {
        final CouponApplyDomain domain = CouponApplyDomain.of(
            CouponApplyCondition.create(List.of("p1"), List.of(), List.of(), null)
        );
        assertThat(domain.isSatisfied(null, "c1", "b1", null)).isFalse();
    }
}
